/*
 * TI DaVinci DM36x pin multiplex
 *
...
struct mux_debug {
	...
	const char *muxnames[DM36X_MUX_NR_MODES];
 };

static const struct mux_debug dm36x_mux_debug[]	= {
...
MUX_DBG(2,	12,	1,	"EM_CLK",	"GIO50",	NULL,		NULL)
MUX_DBG(2,	11,	1,	"EM_ADV",	"GIO51",	NULL,		NULL)
MUX_DBG(2,	10,	1,	"EM_WAIT",	"GIO52",	NULL,		NULL)
MUX_DBG(2,	9,	1,	"EM_WE and EM_OE", "GIO[54:53]",	NULL,	NULL)
MUX_DBG(2,	8,	1,	"EM_CE1",	"GIO55",	NULL,		NULL)
...
MUX_DBG(1,	2,	3,	"GIO91",	"COUT6",	"PWM1",		NULL)
...
#define DM36X_MUX_MAX_NR_FLAGS	10
#define DM36X_MUX_TEST_FLAG(val, mask)				\
	if (((val) & (mask)) == (mask)) {			\
		i++;						\
		flags[i] =  #mask;				\
	}

#define DM36X_MUX_DEFNAME_LEN	16
...
int pinmux_info(void)
{
  void  *base = (void *)DAVINCI_SYSTEM_MODULE_BASE;
	...
	for (i = 0; i < ARRAY_SIZE(dm36x_mux_debug); i++) {
		const struct mux_debug *m = &dm36x_mux_debug[i];
		u32 val;
		int j, mode;
